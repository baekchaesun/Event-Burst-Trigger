...
def apply_triggers_vectorized(
    ...
    if (not centers_entry or not border_coords_entry):
        try:
            raw = np.loadtxt(label_txt_path, dtype=float, ndmin=2)
            if raw.size > 0:
                if not centers_entry:
                    centers_entry = {}
                if not border_coords_entry:
                    border_coords_entry = {}
                for row in raw:
                    if row.shape[0] < 5:
                        continue
                    ...
                    if cls_id not in centers_entry:
                        centers_entry[cls_id] = (x_c_n, y_c_n)

                    if cls_id not in border_coords_entry:
    ...
    if centers_entry:
        for cls_id, (x_norm, y_norm) in centers_entry.items():
            cx, cy = norm_to_abs_xy(x_norm, y_norm, W, H)
            patch = crop_patch_centered(base_frame, cx, cy, radius)
            
            enhanced_patch = enhance_events_naturally(patch, target_event_ratio=1.0)
            
            class_to_patch[cls_id] = enhanced_patch
    else:
        if border_coords_entry:
            for cls_id in border_coords_entry.keys():
  
                coords_list = border_coords_entry[cls_id]
                if coords_list:
        ...
        if not class_to_patch:
            cx, cy = W // 2, H // 2
            patch = crop_patch_centered(base_frame, cx, cy, radius)
            enhanced_patch = enhance_events_naturally(patch, target_event_ratio=1.0)
            class_to_patch[0] = enhanced_patch
        ...
        try:
            ...
            if raw.size > 0:
                for row in raw:
                    ...
                    y_min = max(0, min(H - 1, y_min)); y_max = max(0, min(H - 1, y_max))
        ...
        if border_coords_entry:
            for cls_id, coords_list in border_coords_entry.items():
                ...
                if patch is None:
                    ...
                    cy = (min(ys) + max(ys)) // 2
                    patch = crop_patch_centered(base_frame, cx, cy, radius)
                    patch = enhance_events_naturally(patch, target_event_ratio=1.0)
                    class_to_patch[cls_id] = patch
