...
class BaseTrainer:
    ...
    def resume_training(self, ckpt):
        ...
        # 이미 완료된 체크포인트에서도 계속 학습할 수 있도록 epochs 자동 증가
        if self.epochs < start_epoch:
            LOGGER.info(
                f"{self.model} has been trained for {ckpt['epoch']} epochs. Continuing training for {self.epochs} more epochs.")
            self.epochs = start_epoch + self.epochs  # 추가로 학습할 epochs만큼 더함
        
        if self.resume:
            if start_epoch <= 0:
                LOGGER.warning(
                    f'{self.args.model} checkpoint epoch is invalid ({ckpt.get("epoch", "unknown")}). Starting from epoch 0.')
                start_epoch = 0
            else:
                LOGGER.info(
